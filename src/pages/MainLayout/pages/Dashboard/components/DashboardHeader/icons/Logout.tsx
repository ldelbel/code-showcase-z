interface LogoutIconProps {
  className?: string;
  size?: number;
}

export const LogoutIcon = ({ className = '', size = 24 }: LogoutIconProps) => {
  return (
    <svg 
      width={size} 
      height={size} 
      viewBox="0 0 24 24" 
      fill="none" 
      xmlns="http://www.w3.org/2000/svg"
      className={className}
    >
      <path 
        d="M2 12L0.828697 11.063L0.0790627 12L0.828697 12.937L2 12ZM11 13.5C11.8284 13.5 12.5 12.8284 12.5 12C12.5 11.1716 11.8284 10.5 11 10.5V13.5ZM4.8287 6.06296L0.828697 11.063L3.1713 12.937L7.1713 7.93704L4.8287 6.06296ZM0.828697 12.937L4.8287 17.937L7.1713 16.063L3.1713 11.063L0.828697 12.937ZM2 13.5H11V10.5H2V13.5Z" 
        fill="white"
      />
      <path 
        d="M10 8.13193V7.38851C10 5.77017 10 4.961 10.474 4.4015C10.9479 3.84201 11.7461 3.70899 13.3424 3.44293L15.0136 3.1644C18.2567 2.62388 19.8782 2.35363 20.9391 3.25232C22 4.15102 22 5.79493 22 9.08276V14.9172C22 18.2051 22 19.849 20.9391 20.7477C19.8782 21.6464 18.2567 21.3761 15.0136 20.8356L13.3424 20.5571C11.7461 20.291 10.9479 20.158 10.474 19.5985C10 19.039 10 18.2298 10 16.6115V16.066" 
        stroke="white" 
        strokeWidth="3"
      />
    </svg>
  );
};

export default LogoutIcon;
