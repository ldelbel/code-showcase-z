import { useFetchShareImage } from '@/api/hooks';
...
const Share = ({ userArtifact }: ShareProps) => {
  ...
  if (!userArtifact.archetypeData) {
    return;
  }

  const {
    archetypeData: { combination, virtues, name },
  } = userArtifact;

  const imageUrl =
    userArtifact.framedImageUrl ||
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const shareData: ShareData = {
        title: name,
        text: description,
        url: 'https://zodiac.app',
      };

      const blob = data as Blob;
      const mimeType = blob.type || 'image/jpeg';
      const extension = mimeType.includes('png') ? 'png' : 'jpg';
      const file = new File([blob], `zodic-artifact.${extension}`, {
        type: mimeType,
      });
      ...
      <div className="flex gap-5 justify-center items-start mb-8 pt-5">
        ...
        >
          ...
          <div className="absolute -bottom-14 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            <div
              className="w-[50px] h-[30px] bg-golden-gradient rounded-md flex justify-center items-center cursor-pointer disabled:cursor-not-allowed disabled:opacity-50"
              style={{
              ...
              aria-label={t.shareNotSupported}
